...
const STORES_INIT=[{id:1,name:"信義旗艦店",address:"台北市信義區松仁路28號",phone:"02-2345-6781",hours:"09:00-21:00",revenue:28500,orders:190,customers:156,status:"open"},{id:2,name:"忠孝東路店",address:"台北市大安區忠孝東路四段52號",phone:"02-2345-6782",hours:"09:00-21:00",revenue:22100,orders:148,customers:121,status:"open"},{id:3,name:"西門町店",address:"台北市萬華區漢中街51號",phone:"02-2345-6783",hours:"10:00-22:00",revenue:18900,orders:125,customers:102,status:"open"},{id:4,name:"板橋中山店",address:"新北市板橋區中山路一段46號",phone:"02-2345-6784",hours:"09:00-21:00",revenue:15200,orders:102,customers:89,status:"open"},{id:5,name:"中壢SOGO店",address:"桃園市中壢區元化路357號",phone:"03-4567-890",hours:"10:00-21:30",revenue:12800,orders:85,customers:72,status:"closed"}];
...
function GP({children}){
  ...
  const placeO=(order)=>{setOHist(p=>[order,...p]);setPts(p=>p+order.points);setLastO(order);setTotalCups(p=>p+order.itemCount);const ao={id:`B-${String(Math.floor(Math.random()*9000)+1000)}`,time:new Date().toLocaleTimeString("zh-TW",{hour:"2-digit",minute:"2-digit",hour12:false}),store:stores.find(s=>s.id===selStore)?.name||"",table:`A-${Math.floor(Math.random()*20)+1}`,items:order.items,total:order.total,status:"new",itemRevenues:order.itemRevenues||null};setAOrders(p=>[ao,...p]);setStores(p=>p.map(s=>s.id===selStore?{...s,revenue:s.revenue+order.total,orders:s.orders+order.itemCount,customers:s.customers+1}:s));
...
// ADMIN
// ============================================================
const WR=[{day:"一",revenue:64200},{day:"二",revenue:72800},{day:"三",revenue:58500},{day:"四",revenue:81200},{day:"五",revenue:92400},{day:"六",revenue:115800},{day:"日",revenue:97500}];
const HR=[{hour:"09",orders:48},{hour:"10",orders:112},{hour:"11",orders:180},{hour:"12",orders:248},{hour:"13",orders:220},{hour:"14",orders:192},{hour:"15",orders:208},{hour:"16",orders:152},{hour:"17",orders:168},{hour:"18",orders:140},{hour:"19",orders:112},{hour:"20",orders:72}];
...
function ADash(){const{stores,inEv,getMul,aOrders,menuItems}=useG();const mul=getMul();const ie=inEv();const tR=stores.reduce((s,x)=>s+x.revenue,0);const tO=stores.reduce((s,x)=>s+x.orders,0);const tC=stores.reduce((s,x)=>s+x.customers,0);const tP=tO*mul;
  // Yesterday mock data for comparison
  const yR=Math.round(tR*0.85);const yO=Math.round(tO*0.88);const yC=Math.round(tC*0.9);const yP=Math.round(yO*mul);
  const pct=(t,y)=>{const d=Math.round((t-y)/y*100);return d>=0?`+${d}%`:`${d}%`;};
  const isUp=(t,y)=>t>=y;
  // Daily report
  const[showReport,setShowReport]=useState(false);const[reportRange,setReportRange]=useState("7");
  const reportData=Array.from({length:Number(reportRange)}).map((_,i)=>{const d=new Date();d.setDate(d.getDate()-i);const ds=d.toLocaleDateString("zh-TW",{month:"numeric",day:"numeric"});const rv=Math.round(tR*(0.7+Math.random()*0.6));const od=Math.round(tO*(0.7+Math.random()*0.6));return{date:ds,revenue:rv,orders:od};}).reverse();
...
function ALoy(){const{events,setEvents,inEv,getMul,rewards,setRewards}=useG();const mul=getMul();const ie=inEv();const[er,setER]=useState(false);const[rt,setRT]=useState("每購買1杯集1點（活動期間依倍率）");const[erw,setERW]=useState(null);const[sar,setSAR]=useState(false);const[nr,setNR]=useState({name:"",points:"",icon:"🎁",couponType:"discount",couponValue:"90"});const[ee,setEE]=useState(null);const[sae,setSAE]=useState(false);const[ne,setNE]=useState({name:"",start:"",end:"",multiplier:"2"});
  ...
  return <div style={{padding:"18px 20px 24px",overflow:"auto",flex:1}}>
    {er&&<Mod title="修改規則" onClose={()=>setER(false)} width={400}><FF label="規則" value={rt} onChange={setRT}/><button onClick={()=>setER(false)} style={{width:"100%",padding:12,borderRadius:12,border:"none",background:P,color:"#fff",fontWeight:700,cursor:"pointer"}}>儲存</button></Mod>}
    ...
    <div style={{display:"flex",gap:10,marginBottom:16}}>
      <div style={{flex:1,background:"#fff",borderRadius:12,padding:14,border:`1px solid ${K.border}`}}><div style={{fontWeight:700,marginBottom:10}}>集點規則</div><div style={{padding:10,background:LG,borderRadius:8,marginBottom:8,fontSize:13}}>{rt}</div><div style={{fontSize:11,color:GR,marginBottom:8}}>目前：{ie?`×${mul}（活動中）`:"×1"}</div><button onClick={()=>setER(true)} style={{width:"100%",padding:7,borderRadius:8,border:`1px solid ${P}`,background:"#fff",color:P,fontSize:12,fontWeight:600,cursor:"pointer"}}>修改</button></div>
      <div style={{flex:1,background:`linear-gradient(135deg,${AL},#fff5e0)`,borderRadius:12,padding:14,border:`1px solid ${AC}30`}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}><span style={{fontWeight:700}}>限時活動</span><button onClick={()=>setSAE(true)} style={{padding:"3px 10px",borderRadius:6,border:`1px solid ${K.border}`,background:"#fff",fontSize:10,cursor:"pointer"}}>+ 新增</button></div>{events.map(ev=>{const t=new Date().toISOString().slice(0,10);const on=ev.active&&t>=ev.start&&t<=ev.end;return <div key={ev.id} style={{marginBottom:8}}><span style={{fontSize:10,color:on?K.success:GR,background:on?"#d1fae5":"#f3f4f6",padding:"2px 6px",borderRadius:100}}>{on?"進行中":"未啟用"}</span><div style={{fontSize:15,fontWeight:800,marginTop:4}}>🔥 {ev.name}</div><div style={{fontSize:11,color:GR}}>{ev.start} - {ev.end} · ×{ev.multiplier}</div><button onClick={()=>setEE({...ev})} style={{marginTop:6,padding:"5px 12px",borderRadius:8,border:"none",background:AC,color:"#fff",fontSize:11,fontWeight:600,cursor:"pointer"}}>編輯</button></div>;})}</div>
    </div>

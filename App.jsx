...
function ADash(){const{stores,inEv,getMul,aOrders,menuItems}=useG();const mul=getMul();const ie=inEv();
  ...
  return <div style={{padding:"16px",overflow:"auto",flex:1,WebkitOverflowScrolling:"touch"}}>
...
function AApp({pg,setPG}){const r=()=>{switch(pg){case"dashboard":return <ADash/>;case"orders":return <AOrd/>;case"menu":return <AMenu/>;case"loyalty":return <ALoy/>;case"stores":return <ASto/>;case"members":return <AMem/>;default:return <ADash/>;}};return <div style={{width:"100%",maxWidth:800,margin:"0 auto",display:"flex",flexDirection:"column",flex:1,background:K.lightGray,borderRadius:16,overflow:"hidden",boxShadow:"0 4px 30px rgba(0,0,0,0.08)"}}><style>{`@keyframes adminFade{from{opacity:0;transform:translateY(10px)}to{opacity:1;transform:translateY(0)}}.adminFade{animation:adminFade 0.3s ease-out}`}</style><div key={pg} className="adminFade" style={{flex:1,overflow:"auto",WebkitOverflowScrolling:"touch"}}>{r()}</div><ASide act={pg} nav={setPG}/></div>;}
...
export default function App(){const[v,setV]=useState("customer");const[cPg,setCPg]=useState("home");const[aPg,setAPg]=useState("dashboard");const[ready,setReady]=useState(false);
  ...
  return <GP>
  <div style={{display:"flex",flexDirection:"column",height:"100vh",overflow:"hidden",background:v==="customer"?`linear-gradient(135deg,${PL} 0%,#b7e4c7 50%,${PL} 100%)`:K.lightGray,fontFamily:"'Noto Sans TC',-apple-system,BlinkMacSystemFont,sans-serif"}}><link href="https://fonts.googleapis.com/css2?family=Noto+Sans+TC:wght@300;400;500;600;700;800;900&display=swap" rel="stylesheet"/><div style={{display:"flex",justifyContent:"center",padding:"12px 0 8px",flexShrink:0}}><div style={{display:"flex",background:DK,borderRadius:12,padding:3}}><button onClick={()=>setV("customer")} style={{padding:"7px 18px",borderRadius:9,border:"none",background:v==="customer"?"#fff":"transparent",color:v==="customer"?DK:"rgba(255,255,255,0.6)",fontSize:13,fontWeight:700,cursor:"pointer"}}>📱 顧客端</button><button onClick={()=>setV("admin")} style={{padding:"7px 18px",borderRadius:9,border:"none",background:v==="admin"?"#fff":"transparent",color:v==="admin"?DK:"rgba(255,255,255,0.6)",fontSize:13,fontWeight:700,cursor:"pointer"}}>💻 商家後台</button></div></div><div style={{display:v==="customer"?"flex":"none",justifyContent:"center",flex:1,padding:"0 20px 20px",overflow:"hidden"}}><CApp pg={cPg} setPG={setCPg}/></div><div style={{display:v==="admin"?"flex":"none",flex:1,justifyContent:"center",padding:"0 20px 20px",overflow:"hidden"}}><AApp pg={aPg} setPG={setAPg}/></div></div></GP>;}

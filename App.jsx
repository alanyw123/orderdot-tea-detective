...
function CMenu({nav}){
  ...
  // Item detail
  if(si)return <div style={{flex:1,overflow:"auto",touchAction:"auto",WebkitOverflowScrolling:"touch",background:"#fff",position:"relative"}}><Toast m={toast} s={!!toast}/><div style={{height:170,background:`linear-gradient(135deg,${PL},#b7e4c7)`,display:"flex",alignItems:"center",justifyContent:"center",position:"relative"}}><span style={{fontSize:70}}>{getDrinkIcon(si.id)}</span><button onClick={()=>setSI(null)} style={{position:"absolute",top:12,left:12,width:36,height:36,borderRadius:"50%",background:"rgba(0,0,0,0.3)",border:"none",color:"#fff",fontSize:20,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>←</button></div><div style={{padding:"20px 20px 140px"}}><div style={{fontSize:22,fontWeight:800,color:DK}}>{si.name}</div><div style={{fontSize:13,color:GR,marginTop:4}}>{si.desc}</div><div style={{fontSize:12,color:P,marginTop:4,fontWeight:600}}>🪙 集{mul}點{ie?" 🔥":""}</div>
  ...
  if(sc)return <div style={{flex:1,overflow:"auto",touchAction:"auto",WebkitOverflowScrolling:"touch",background:"#fafbfc"}}><div style={{padding:"16px 20px",display:"flex",alignItems:"center",gap:12,borderBottom:"1px solid #e5e7eb",background:"#fff"}}><button onClick={()=>setSC(false)} style={{background:"none",border:"none",fontSize:20,cursor:"pointer"}}>←</button><span style={{fontSize:18,fontWeight:800}}>購物車</span></div><div style={{margin:"16px 20px",background:"#fff",borderRadius:14,padding:"14px 16px",border:"1px solid #f0f0f0"}}><div style={{fontSize:14,fontWeight:700}}>📍 {BN} {st?.name}</div></div>
    {pl?<div style={{display:"flex",flexDirection:"column",alignItems:"center",padding:"60px 20px",gap:16}}><div style={{fontSize:64}}>✅</div><div style={{fontSize:20,fontWeight:800}}>訂單已送出！</div></div>:<>
      ...
      <div style={{margin:"10px 20px 20px",background:"#fff",borderRadius:14,padding:"16px"}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:8,fontSize:14,color:GR}}><span>小計（{td}杯）</span><span>${ct}</span></div>{disc>0&&<div style={{display:"flex",justifyContent:"space-between",marginBottom:8,fontSize:14,color:K.danger}}><span>折抵</span><span>-${disc}</span></div>}{cc?.type==="redeemed_upgrade"&&!cart.some(c=>c.size==="L")&&<div style={{display:"flex",justifyContent:"space-between",marginBottom:8,fontSize:14,color:P}}><span>免費升級</span><span>$0</span></div>}<div style={{display:"flex",justifyContent:"space-between",marginBottom:8,fontSize:14,color:P}}><span>點數{ie?` (🔥${mul}倍)`:""}</span><span>+{pe}點</span></div><div style={{borderTop:"1px solid #e5e7eb",paddingTop:10,display:"flex",justifyContent:"space-between",fontSize:18,fontWeight:800}}><span>合計</span><span>${ft}</span></div></div>
      <div style={{position:"sticky",bottom:0,padding:"12px 20px 16px",background:"#fff",borderTop:"1px solid #e5e7eb"}}><button onClick={dpl} style={{width:"100%",background:P,color:"#fff",border:"none",borderRadius:14,padding:"16px",fontSize:17,fontWeight:700,cursor:"pointer"}}>確認送出 ${ft}</button></div>
    </>}</div>;

  // Menu list
  return <div style={{flex:1,overflow:"auto",touchAction:"auto",WebkitOverflowScrolling:"touch",background:"#fafbfc"}}><Toast m={toast} s={!!toast}/><div style={{padding:"16px 20px 12px",background:"#fff"}}><div style={{fontSize:12,color:GR}}>📍 {BN} {st?.name}</div><div style={{display:"flex",justifyContent:"space-between",marginTop:4}}><div style={{fontSize:20,fontWeight:800}}>菜單</div>{ie&&<div style={{fontSize:11,color:AC,fontWeight:700,background:AL,padding:"4px 10px",borderRadius:100}}>🔥 每杯{mul}點</div>}</div></div>
...
function CTrk({nav}){const{lastO,inEv,getMul,stores,selStore,profile,menuItems}=useG();const mul=getMul();const ie=inEv();const[s,setS]=useState(0);const[fortune]=useState(()=>TEA_FORTUNES[Math.floor(Math.random()*TEA_FORTUNES.length)]);const[showCard,setShowCard]=useState(false);useEffect(()=>{const a=setTimeout(()=>setS(1),2000),b=setTimeout(()=>setS(2),5000);return()=>{clearTimeout(a);clearTimeout(b);};},[]);
  ...
  if(showCard)return <div style={{flex:1,overflow:"auto",touchAction:"auto",WebkitOverflowScrolling:"touch",background:"#111",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:20}}>
...
function CApp({pg,setPG}){const{cart}=useG();const[animClass,setAnimClass]=useState("fadeIn");const prevPg=useCallback((id)=>{setAnimClass("fadeOut");setTimeout(()=>{setPG(id);setAnimClass("fadeIn");},150);},[setPG]);
  ...
  return <div style={{width:"100%",maxWidth:375,height:"100%",background:"#fff",borderRadius:24,boxShadow:"0 20px 60px rgba(0,0,0,0.15)",overflow:"hidden",display:"flex",flexDirection:"column",position:"relative"}}>
    ...
    <div style={{position:"absolute",top:0,left:"50%",transform:"translateX(-50%)",width:120,height:28,background:DK,borderRadius:"0 0 16px 16px",zIndex:50}}><div style={{position:"absolute",top:10,left:"50%",transform:"translateX(-50%)",width:50,height:4,background:"#555",borderRadius:100}}/></div>
    <div style={{padding:"6px 20px 4px",fontSize:13,fontWeight:600,color:DK,display:"flex",justifyContent:"space-between",flexShrink:0}}><span>14:32</span><div style={{width:25,height:12,border:`1.5px solid ${DK}`,borderRadius:3,display:"flex",alignItems:"center",padding:1}}><div style={{width:"75%",height:"100%",background:DK,borderRadius:1.5}}/></div></div><div key={pg} className={animClass} style={{flex:1,display:"flex",flexDirection:"column",minHeight:0}}>{r()}</div>{pg!=="tracking"&&<div style={{display:"flex",borderTop:"1px solid #e5e7eb",background:"#fff",paddingBottom:6,paddingTop:6,flexShrink:0}}>{tabs.map(t=><button key={t.id} onClick={()=>prevPg(t.id)} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:2,background:"none",border:"none",cursor:"pointer",position:"relative"}}><span style={{fontSize:18,filter:n===t.id?"none":"grayscale(1)",opacity:n===t.id?1:0.5}}>{t.i}</span>{t.id==="cart"&&cart.length>0&&<div style={{position:"absolute",top:-2,right:"50%",marginRight:-14,background:K.danger,color:"#fff",fontSize:8,fontWeight:800,width:14,height:14,borderRadius:"50%",display:"flex",alignItems:"center",justifyContent:"center"}}>{cart.length}</div>}<span style={{fontSize:9,fontWeight:n===t.id?700:400,color:n===t.id?P:GR}}>{t.l}</span></button>)}</div>}</div>;
}
...
function ADash(){const{stores,inEv,getMul,aOrders,menuItems,resetAll}=useG();const mul=getMul();const ie=inEv();
  ...
  return <div style={{padding:"16px",overflow:"auto",flex:1,touchAction:"auto",WebkitOverflowScrolling:"touch"}}>
    ...
    <div style={{background:"#fff",borderRadius:10,padding:14,border:`1px solid ${K.border}`,marginBottom:14}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}><span style={{fontSize:13,fontWeight:700}}>本週營收</span><span style={{fontSize:9,color:GR}}>🟠 今日</span></div><ResponsiveContainer width="100%" height={160}><BarChart data={weekData}><CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0"/><XAxis dataKey="day" tick={{fontSize:10,fill:GR}}/><YAxis tick={{fontSize:9,fill:GR}} width={45}/><Tooltip formatter={(v)=>[`$${v.toLocaleString()}`,"營收"]}/><Bar dataKey="revenue" radius={[4,4,0,0]}>{weekData.map((e,i)=><Cell key={i} fill={e.isToday?AC:P}/>)}</Bar></BarChart></ResponsiveContainer></div>
    {/* Category pie + Hourly orders side by side, stack on very small */}
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:14}}>
      <div style={{background:"#fff",borderRadius:10,padding:14,border:`1px solid ${K.border}`}}><div style={{fontSize:13,fontWeight:700,marginBottom:6}}>品類佔比</div>{(()=>{const total=CS.reduce((s,c)=>s+c.value,0);const cx=70,cy=65,R=50,r=24;let cumAngle=-90;return <svg width="140" height="130" viewBox="0 0 140 130">{CS.map((c,i)=>{const angle=(c.value/total)*360;const startAngle=cumAngle;const endAngle=cumAngle+angle;const midAngle=startAngle+angle/2;cumAngle=endAngle;const toRad=a=>a*Math.PI/180;const x1=cx+R*Math.cos(toRad(startAngle));const y1=cy+R*Math.sin(toRad(startAngle));const x2=cx+R*Math.cos(toRad(endAngle));const y2=cy+R*Math.sin(toRad(endAngle));const ix1=cx+r*Math.cos(toRad(endAngle));const iy1=cy+r*Math.sin(toRad(endAngle));const ix2=cx+r*Math.cos(toRad(startAngle));const iy2=cy+r*Math.sin(toRad(startAngle));const large=angle>180?1:0;const d=`M${x1} ${y1} A${R} ${R} 0 ${large} 1 ${x2} ${y2} L${ix1} ${iy1} A${r} ${r} 0 ${large} 0 ${ix2} ${iy2} Z`;const labelR=(R+r)/2;const lx=cx+labelR*Math.cos(toRad(midAngle));const ly=cy+labelR*Math.sin(toRad(midAngle));return <g key={i}><path d={d} fill={c.color}/><text x={lx} y={ly-4} textAnchor="middle" dominantBaseline="central" fill="#fff" stroke="#00000040" strokeWidth="0.3" style={{fontSize:8,fontWeight:800}}>{c.name}</text><text x={lx} y={ly+6} textAnchor="middle" dominantBaseline="central" fill="#fff" stroke="#00000040" strokeWidth="0.3" style={{fontSize:7,fontWeight:700}}>{c.value}%</text></g>;})}</svg>;})()}</div>
      <div style={{background:"#fff",borderRadius:10,padding:14,border:`1px solid ${K.border}`}}><div style={{fontSize:13,fontWeight:700,marginBottom:2}}>今日訂單時段</div><div style={{fontSize:9,color:GR,marginBottom:6}}>合計：{tO} 筆</div><ResponsiveContainer width="100%" height={120}><LineChart data={hourData}><CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0"/><XAxis dataKey="hour" tick={{fontSize:8,fill:GR}} interval={1}/><YAxis tick={{fontSize:8,fill:GR}} width={30}/><Tooltip formatter={(v)=>[`${v} 筆`,"訂單"]}/><Line type="monotone" dataKey="訂單數" stroke={P} strokeWidth={2} dot={{fill:P,r:1.5}}/></LineChart></ResponsiveContainer></div>
    </div>
    {/* Hot items - full width */}
    <div style={{background:"#fff",borderRadius:10,padding:14,border:`1px solid ${K.border}`,marginBottom:14}}><div style={{fontSize:13,fontWeight:700,marginBottom:8}}>熱銷排行</div>{hotItems.map((it,i)=><div key={i} style={{display:"flex",justifyContent:"space-between",padding:"5px 0",borderBottom:i<hotItems.length-1?`1px solid ${K.border}`:"none"}}><span style={{fontSize:12,fontWeight:600}}>{i+1}. {it.name}</span><span style={{fontSize:12,fontWeight:700}}>${it.revenue.toLocaleString()}</span></div>)}</div>
    {/* Store table */}
    <div style={{background:"#fff",borderRadius:10,padding:14,border:`1px solid ${K.border}`}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}><span style={{fontSize:13,fontWeight:700}}>各門市</span>{ie&&<span style={{fontSize:9,color:AC,background:AL,padding:"2px 6px",borderRadius:100}}>×{mul}</span>}</div><div style={{overflowX:"auto",WebkitOverflowScrolling:"touch"}}><div style={{minWidth:380}}><div style={{display:"grid",gridTemplateColumns:"2fr 1fr 1fr 1fr 60px",gap:4,padding:"5px 0",borderBottom:`1px solid ${K.border}`,fontSize:9,color:GR,fontWeight:600}}><span>門市</span><span style={{textAlign:"right"}}>營收</span><span style={{textAlign:"right"}}>訂單</span><span style={{textAlign:"right"}}>點數</span><span style={{textAlign:"right"}}>狀態</span></div>{stores.map(s=><div key={s.id} style={{display:"grid",gridTemplateColumns:"2fr 1fr 1fr 1fr 60px",gap:4,padding:"7px 0",borderBottom:`1px solid ${K.border}`,alignItems:"center"}}><div style={{fontSize:11,fontWeight:600}}>{s.name}</div><div style={{textAlign:"right",fontSize:11,fontWeight:700}}>${s.revenue.toLocaleString()}</div><div style={{textAlign:"right",fontSize:11}}>{s.orders}</div><div style={{textAlign:"right",fontSize:11,color:P,fontWeight:600}}>{(s.cups*mul).toLocaleString()}</div><div style={{textAlign:"right"}}><span style={{fontSize:8,color:s.status==="open"?K.success:GR,background:s.status==="open"?"#d1fae5":"#f3f4f6",padding:"2px 5px",borderRadius:100}}>{s.status==="open"?"營業":"休息"}</span></div></div>)}</div></div></div>
  </div>;
}
...
function AApp({pg,setPG}){const r=()=>{switch(pg){case"dashboard":return <ADash/>;case"orders":return <AOrd/>;case"menu":return <AMenu/>;case"loyalty":return <ALoy/>;case"stores":return <ASto/>;case"members":return <AMem/>;default:return <ADash/>;}};return <div style={{width:"100%",maxWidth:800,margin:"0 auto",display:"flex",flexDirection:"column",height:"100%",background:K.lightGray,borderRadius:16,overflow:"hidden",boxShadow:"0 4px 30px rgba(0,0,0,0.08)"}}><style>{`@keyframes adminFade{from{opacity:0;transform:translateY(10px)}to{opacity:1;transform:translateY(0)}}.adminFade{animation:adminFade 0.3s ease-out}`}</style><div key={pg} className="adminFade" style={{flex:1,overflow:"auto",touchAction:"auto",WebkitOverflowScrolling:"touch"}}>{r()}</div><ASide act={pg} nav={setPG}/></div>;}
...
export default function App(){const[v,setV]=useState("customer");const[cPg,setCPg]=useState("home");const[aPg,setAPg]=useState("dashboard");const[ready,setReady]=useState(false);
  ...
  return <GP>
  <style>{`html,body{margin:0;padding:0;height:100%;overflow:hidden;position:fixed;width:100%;touch-action:none}#root{height:100%;overflow:hidden}`}</style>
  <div style={{position:"fixed",inset:0,display:"flex",flexDirection:"column",background:v==="customer"?`linear-gradient(135deg,${PL} 0%,#b7e4c7 50%,${PL} 100%)`:K.lightGray,fontFamily:"'Noto Sans TC',-apple-system,BlinkMacSystemFont,sans-serif"}}><link href="https://fonts.googleapis.com/css2?family=Noto+Sans+TC:wght@300;400;500;600;700;800;900&display=swap" rel="stylesheet"/><div style={{display:"flex",justifyContent:"center",padding:"8px 0 6px",flexShrink:0}}><div style={{display:"flex",background:DK,borderRadius:12,padding:3}}><button onClick={()=>setV("customer")} style={{padding:"7px 18px",borderRadius:9,border:"none",background:v==="customer"?"#fff":"transparent",color:v==="customer"?DK:"rgba(255,255,255,0.6)",fontSize:13,fontWeight:700,cursor:"pointer"}}>📱 顧客端</button><button onClick={()=>setV("admin")} style={{padding:"7px 18px",borderRadius:9,border:"none",background:v==="admin"?"#fff":"transparent",color:v==="admin"?DK:"rgba(255,255,255,0.6)",fontSize:13,fontWeight:700,cursor:"pointer"}}>💻 商家後台</button></div></div><div style={{display:v==="customer"?"flex":"none",justifyContent:"center",flex:1,minHeight:0,padding:"0 12px 8px"}}><CApp pg={cPg} setPG={setCPg}/></div><div style={{display:v==="admin"?"flex":"none",flex:1,minHeight:0,padding:"0 8px 8px"}}><AApp pg={aPg} setPG={setAPg}/></div></div></GP>;}

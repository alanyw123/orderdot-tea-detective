...
function CMenu({nav}){
  ...
  // Cart
  if(sc)return <div style={{flex:1,overflow:"auto",touchAction:"auto",WebkitOverflowScrolling:"touch",background:"#fafbfc",position:"relative"}}><div style={{padding:"16px 20px",display:"flex",alignItems:"center",gap:12,borderBottom:"1px solid #e5e7eb",background:"#fff"}}><button onClick={()=>setSC(false)} style={{background:"none",border:"none",fontSize:20,cursor:"pointer"}}>←</button><span style={{fontSize:18,fontWeight:800}}>購物車</span></div><div style={{margin:"16px 20px",background:"#fff",borderRadius:14,padding:"14px 16px",border:"1px solid #f0f0f0"}}><div style={{fontSize:14,fontWeight:700}}>📍 {BN} {st?.name}</div></div>
  ...
  // Menu list
  return <div style={{flex:1,overflow:"auto",touchAction:"auto",WebkitOverflowScrolling:"touch",background:"#fafbfc",position:"relative"}}><Toast m={toast} s={!!toast}/><div style={{padding:"16px 20px 12px",background:"#fff"}}><div style={{fontSize:12,color:GR}}>📍 {BN} {st?.name}</div><div style={{display:"flex",justifyContent:"space-between",marginTop:4}}><div style={{fontSize:20,fontWeight:800}}>菜單</div>{ie&&<div style={{fontSize:11,color:AC,fontWeight:700,background:AL,padding:"4px 10px",borderRadius:100}}>🔥 每杯{mul}點</div>}</div></div>
